...
function CollectiveC.Notification(theType, message)
    ...
    elseif Config.NotificationType.client == 'ox_lib' then
        if GetResourceState('ox_lib') == 'started' then
            if theType == 1 then
                lib.notify({title = 'LICENSE PLATE', description = message, type = 'success', duration = 5000})
            elseif theType == 2 then
                lib.notify({title = 'LICENSE PLATE', description = message, type = 'inform', duration = 5000})
            elseif theType == 3 then
                lib.notify({title = 'LICENSE PLATE', description = message, type = 'error', duration = 5000})
            end
        end
    elseif Config.NotificationType.client == 't-notify' then
        if theType == 1 then
            exports['t-notify']:Custom({style = 'success', duration = 5000, title = 'LICENSE PLATE', message = message, sound = false})
        elseif theType == 2 then
            exports['t-notify']:Custom({style = 'info', duration = 5000, title = 'LICENSE PLATE', message = message, sound = false})
        elseif theType == 3 then
            exports['t-notify']:Custom({style = 'error', duration = 5000, title = 'LICENSE PLATE', message = message, sound = false})
        end
    elseif Config.NotificationType.client == 'col_notify' then
        if theType == 1 then
            exports['col_notify']:Custom({style = 'success', duration = 5000, title = 'LICENSE PLATE', message = message, sound = false})
        elseif theType == 2 then
            exports['col_notify']:Custom({style = 'info', duration = 5000, title = 'LICENSE PLATE', message = message, sound = false})
        elseif theType == 3 then
            exports['col_notify']:Custom({style = 'error', duration = 5000, title = 'LICENSE PLATE', message = message, sound = false})
        end
    ...
    elseif Config.NotificationType.client == 'col_notify_new' then
        if theType == 1 then
            exports["col_notify"]:Notify('ann', 5000, message, 'LICENSE PLATE')
        elseif theType == 2 then
            exports["col_notify"]:Notify('ann', 5000, message, 'LICENSE PLATE')
        elseif theType == 3 then
            exports["col_notify"]:Notify('ann', 5000, message, 'LICENSE PLATE')
    ...
    elseif Config.NotificationType.client == 'okokNotify' then
        if theType == 1 then
            exports['okokNotify']:Alert("LICENSE PLATE", message, 5000, 'success')
        elseif theType == 2 then
            exports['okokNotify']:Alert("LICENSE PLATE", message, 5000, 'info')
        elseif theType == 3 then
            exports['okokNotify']:Alert("LICENSE PLATE", message, 5000, 'error')
        end
